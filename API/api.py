...
from portfolio.models import UnidadeCurricular, Projeto, Tecnologia
from portfolio.schemas import UnidadeCurricularOut, UnidadeCurricularIn, ProjetoOut, ProjetoIn, TecnologiaOut, TecnologiaIn
...
def lista_UnidadesCurriculares(request, ano: int = None, page: int = 1, page_size: int = 10):
      ...
      return 200, qs[inicio:fim] # if page_size==5, 1º qs[0:5], 2º qs[5:10], 3º qs[10:15] e assim por diante, primero inclusivo, segundo exclusivo.
...
def lista_Projetos(request, ano_realizacao: int = None, page: int = 1, page_size: int = 10):
    qs = Projeto.objects.all()

    if ano_realizacao is not None:
        qs = qs.filter(ano_realizacao=ano_realizacao)

    inicio = (page - 1) * page_size
    fim = inicio + page_size

    return 200, qs[inicio:fim]
...
def lista_Tecnologias(request, categoria: str = None, page: int = 1, page_size: int = 10):
